    ...
    static get methods() {
        return {
            ...
            clickHandler() {
                ...
                if (this.$.items.attrs().match("expand")) {
                    __hide__(this);
                } else {
    ...
    render() {
        return /*html*/ `
            <main id="xo-container">
                ...
                <div id="xo-items">
                    <input type="text" id="xo-search" placeholder="Search..." (input)="{{>inputHandler()}}" />
                    <slot></slot>
                </div>
            </main>
            @{{if info && infotype}}
...
function __click__(self, e) {
    if (e.target !== self) {
        self.$.items.attrs().del("expand");
        self.$.btn.attrs().del("active");
        self.querySelectorAll("xo-select-item").forEach(e => {
            e.setAttribute("disabled", "");
            e.style.display = "";
        });
        self.$.container.class().del("focus");
        self.$.search.attrs().set("disabled");
        self.$.search.val("");
        if (self.label) {
...
function __show__(self) {
    ...
    self.$.container.class().add("focus");
    self.$.search.attrs().del("disabled");
    self.$.btn.attrs().set("active");
    self.$.items.attrs().set("expand");
}