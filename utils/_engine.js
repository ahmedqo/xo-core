...
var { _typeof, _slicedToArray, _toConsumableArray } = require("./_runtime");
...
function filter(v, f, c) {
    ...
    switch (f.toLowerCase().trim()) {
        ...
        case "camel":
            res =
                v +
                '.trim().replace(/[-/_s.]/g," ").split(" ").reduce((s,e,i)=>{if(i===0) return s + e.toLowerCase();return s + e[0].toUpperCase() + e.slice(1).toLowerCase();},"")';
            break;
        case "decamel":
...
function addVariable(line, js) {
    ...
    if (js) {
        ...
        if (line.startsWith(">")) {
            ...
            var fun = data.shift();
            data = data.map(d => d.startsWith("$") ? '"+' + d.slice(1) + '+"' : d);
            data.splice(0, 0, '"+toJson(this)+"');
            var props = data.join(',')
            code = type ? 'r.push("("+' + fun + '.toString().replaceAll("\\"","\'")+")(' + props + ')");' : "r.push((" + fun + ")(" + props.replaceAll('"+', "").replaceAll('+"', "") + "));";
        } else {
            var _fill2 = _fill(line);

            var _fill3 = _slicedToArray(_fill2, 2);
...
function add(line, js) {
    ...
    if (js) {
        ...
        var _fill3 = _fill(line);

        var _fill4 = _slicedToArray(_fill3, 2);
...
async function shape(html, data) {
    ...
    code += "function toJson(el){" +
        "var sobj = '', i;" +
        "for (i in el)" +
        "if (el.hasOwnProperty(i))" +
        "sobj += '\"' + i + '\":' + (typeof el[i] == 'function' ?" +
        "el[i].toString() : JSON.stringify(el[i])) + ',';" +
        "return ('{' + sobj + '}').replaceAll(\"\\\"\", \"\'\")" +
        "}"
    var _block = block(html);

    var _block2 = _slicedToArray(_block, 2);
    ...
    code += 'return r.join("");';
    code = "with(obj || {}){return (async()=>{" + code + "})()}";
    return code;
}
...
function render(html, data) {
    try {
        var t = new Function("obj", "ctx", html).call(data, data || {});
        return t;
    } catch (e) {
...
module.exports = function(data, vars) {
    if (Array.isArray(data)) {
        return (async function(data, vars) {
            vars = Array.isArray(vars) ? vars : [vars];
            data = Array.isArray(data) ?
                data
                .reduce(function(c, e, i) {
                    return [].concat(_toConsumableArray(c), [e, vars[i] || ""]);
                }, [])
                .join("") :
                data;
        ...
        return function(code, vars) {
            return (async function(code, vars, data) {
                vars = Array.isArray(vars) ? vars : [vars];
